package com.cat.component.redis.util;
...
public class RedisTemplates {
    ...
    public RedisTemplates(RedisTemplate<String, Object> redisTemplate) {
    ...
    /**
     * 只有在 key 不存在时设置 key 的值 ，并将 key 的过期时间设为 timeout
     *
     * @param key   键
     * @param value 值
     * @param timeout 过期时间
     ...
     * @return 之前已经存在返回 false, 不存在返回 true
     */
    public Boolean setIfAbsentEx(String key, Object value, long timeout, TimeUnit unit) {
     ...
     * @return 增加后的值
     */
    public synchronized Long incrByAndExpire(String key, long timeout, TimeUnit unit) {
        Long increment = this.incrBy(key);
        // 第一次设置过期时间
        if (null == increment || increment < 2) {
            this.expire(key, timeout, unit);
            Long time = this.getExpire(key, unit);
            if (time != null && time == -1) {
                // 设置失败重新设置过期时间
                this.expire(key, timeout, unit);
            }
     ...
     * @param suffix 后缀
     */
    public static String recudeCacheKey(String prefix, Object... suffix) {
